...
def run_tfasty(
    ...
    for cluster_idx, exon_cluster in enumerate(exon_clusters):
        ...
        if protein not in protein_seqs:
            logger.warning(
                f"Protein {protein} not found in protein file. Skipping cluster {cluster_idx+1}."
            )
            continue

        if chrom not in genome_seqs:
            logger.warning(
                f"Chromosome {chrom} not found in genome file. Skipping cluster {cluster_idx+1}."
            )
            continue
        ...
        for exon_idx, exon in enumerate(exon_cluster):
            try:
                ...
                ) as exon_file, tempfile.NamedTemporaryFile(
                    mode="w", suffix=".fa", delete=False
                ) as prot_file:
                ...
                alignment_out = os.path.join(
                    tfasty_output_dir, f"{exon_id}_alignment.txt"
                )
                ...
                parsed_alignment = _parse_tfasty_output(
                    alignment_out, exon_info, logger
                )
                ...
                logger.error(
                    f"Error processing exon {exon_idx+1} in cluster {cluster_idx+1}: {e}"
                )
                import traceback
        ...
        if not exon_results:
            logger.warning(
                f"No successful alignments for cluster {cluster_idx+1}. Skipping."
            )
            continue

        # Now merge exon alignments into a pseudogene
        merged_pseudogene = _merge_exons(
            pseudogene_id, exon_results, chrom, protein, strand, protein_seqs
        )

        # Filter pseudogenes based on evalue, coverage and identity
        if (
            merged_pseudogene["evalue"] <= evalue
            and merged_pseudogene["coverage"] >= coverage
            and merged_pseudogene["identity"] >= identity
        ):
            # Add to our results
    ...
    logger.info(
        f"Completed tfasty re-alignment: {len(pseudogene_results)} pseudogenes remained"
    )
    ...
    try:
        ...
        while i < len(content) and not content[i].startswith(">>><<<"):
            ...
            if line.startswith(f">>{exon_id}") or line.startswith(">--"):
                ...
                while (
                    i < len(content)
                    and not content[i].startswith(f">>{exon_id}")
                    ...
                    # Get to the protein section
                        ...
                        while i < len(content) and content[i].startswith(";"):
                            ...
                            if line.startswith("; al_start:"):
                                alignment["protein_range"]["start"] = int(
                                    line.split(":", 1)[1].strip()
                                )
                            elif line.startswith("; al_stop:"):
                                alignment["protein_range"]["end"] = int(
                                    line.split(":", 1)[1].strip()
                                )
                            i += 1
                        ...
                        while i < len(content) and content[i].startswith(";"):
                            line = content[i].strip()
                            if line.startswith("; al_start:"):
                                alignment["dna_range"]["start"] = int(
                                    line.split(":", 1)[1].strip()
                                )
                            elif line.startswith("; al_stop:"):
                                alignment["dna_range"]["end"] = int(
                                    line.split(":", 1)[1].strip()
                                )
                            i += 1
                ...
                if alignment["protein_seq"] and alignment["dna_seq"]:
                    ...
                    alignment["frameshifts"] = (
                        forward_frameshifts + backward_frameshifts
                    )
...
def _merge_exons(pseudogene_id, exon_results, chrom, protein, strand, protein_seqs):
    ...
    for exon_idx, exon in enumerate(exon_results):
        ...
        # Get DNA coordinates
        dna_start = min(
            alignment["dna_range"].get("start", 0), alignment["dna_range"].get("end", 0)
        )
        dna_end = max(
            alignment["dna_range"].get("start", 0), alignment["dna_range"].get("end", 0)
        )
        ...
        merged_pseudogene["evalue"] = min(
            merged_pseudogene["evalue"], alignment["evalue"]
        )
        ...
        exon_prot_start = min(
            alignment["protein_range"].get("start", 0),
            alignment["protein_range"].get("end", 0),
        )
        exon_prot_end = max(
            alignment["protein_range"].get("start", 0),
            alignment["protein_range"].get("end", 0),
