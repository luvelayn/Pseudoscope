...
def classify_pseudogenes(pseudogene_results, genome_seqs, logger):
    ...
    genome_file : str
        Path to the genome FASTA file used to check for poly-A tails.
    ...
    sorted_pseudogenes = sorted(pseudogene_results, key=lambda pg: (pg['chrom'], pg['start']))
        ...
        # Check if single exon
            ...
            if pg['strand'] == '+':
                ...
                for other_pg in pg_by_chrom.get(chrom, []):
                    if other_pg['start'] > search_start and other_pg['start'] < search_end:
