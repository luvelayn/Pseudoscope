def _find_sites(sequence, motif):
    ...
    for i in range(len(sequence) - motif_len + 1):
        if sequence[i:i+motif_len].lower() == motif.lower():
            positions.append(i)
    return positions
...
def refine_exon_structure(pseudogenes, genome_seqs, logger):
    ...
    If valid splice sites aren't found between exons, merge them into a single exon.
    ...
    --------
    list of dict
        Refined pseudogenes with updated exon structure
    """
    ...
    for pg_idx, pg in enumerate(pseudogenes):
        ...
        for i, exon in enumerate(exons):
            ...
            if i == len(exons) - 1:
                if current_merged_exon != exon:  # If we're already merging
                    ...
                    current_merged_exon['identity'] = (current_merged_exon['identity'] + exon['identity']) / 2
            ...
            if strand == '+':
                ...
                donor_region_end = min(next_exon['start'], exon['end'] + SPLICE_CONTEXT + 2)
                donor_region = genome_seqs[chrom][donor_region_start:donor_region_end]
                
                # Extract sequence around acceptor site (next exon start)
                acceptor_region_start = max(exon['end'], next_exon['start'] - SPLICE_CONTEXT - 2)
                acceptor_region_end = min(next_exon['end'], next_exon['start'] + SPLICE_CONTEXT)
                acceptor_region = genome_seqs[chrom][acceptor_region_start:acceptor_region_end]
                ...
                intron_start = donor_region_start + donor_positions[-1] if donor_positions else None
                intron_end = acceptor_region_start + acceptor_positions[0] + 1 if acceptor_positions else None
                
                # Check if there are both splice sites
                has_canonical_splice = intron_start is not None and intron_end is not None
                ...
                donor_region_start = max(exon['end'], next_exon['start'] - SPLICE_CONTEXT - 2)
                donor_region_end = min(next_exon['end'], next_exon['start'] + SPLICE_CONTEXT)
                donor_region = genome_seqs[chrom][donor_region_start:donor_region_end]
                
                # Extract sequence around acceptor site (current exon end)
                acceptor_region_start = max(exon['start'], exon['end'] - SPLICE_CONTEXT)
                acceptor_region_end = min(next_exon['start'], exon['end'] + SPLICE_CONTEXT + 2)
                acceptor_region = genome_seqs[chrom][acceptor_region_start:acceptor_region_end]
                
                # We look for AC and CT in the sequence (reverse complement of GT and AG)
                donor_positions = _find_sites(donor_region, _reverse_complement(DONOR_SITE))
                acceptor_positions = _find_sites(acceptor_region, _reverse_complement(ACCEPTOR_SITE))

                # Determine the start and end positions of the intron
                intron_start = acceptor_region_start + acceptor_positions[-1] if acceptor_positions else None
                intron_end = donor_region_start + donor_positions[0] + 1 if donor_positions else None
            ...
            if has_canonical_splice:
                ...
                current_merged_exon['identity'] = (current_merged_exon['identity'] + next_exon['identity']) / 2
        ...
        if refined_exons:
            ...
            avg_identity = sum(exon['identity'] for exon in refined_exons) / len(refined_exons)
